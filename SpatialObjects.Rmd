...
#### stars package
The [`stars` package](https://github.com/r-spatial/stars) is a new package for 'spatiotemporal arrays, raster and vector data cubes' that is meant to be an improvement on and replacement for the `raster` package.  We'll just demonstrate using it to read in, examine and plot some data.  It's worth reading the [package articles](https://r-spatial.github.io/stars/) and discussion on raster and vector datacubes in [Spatial Data Science](https://keen-swartz-3146c4.netlify.com/raster.html).  The big advantages to `stars` are: integration with `sf`, can scale beyond local disk size (which is limit for `raster`), and it handles array data with additional dimensions for time, spectral band and sensor dimensions.
```{r stars, message=FALSE, warning=FALSE, error=FALSE}
library(stars)
# landsat
...
# example from stars help to show more advanced functionality reading and plotting netcdf:
...
We can quickly discover supported I/O vector formats either via `rgdal` or `sf`.  These both list drivers available for GDAL (geospatial data abstraction library).  GDAL is the engine for working with spatial data in R, as well as in Python, QGIS, PostGIS, and numerous other software applications and projects.
