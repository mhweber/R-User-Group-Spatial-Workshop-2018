...
With `sf`, you can read geojson files online directly.  I'm a visitor, so I want to learn a bit more about Durham.  Below we load some spatial data (geojson files) directly in `sf` objects from [Open Data Durham](https://opendurham.nc.gov/explore/?sort=modified):
...
Part of the fun (or consternation) of R, and spatial work in R, is the pace of development of new packages.  I enjoy scanning for new spatial packages and trying to incorporate into my workflow - [tidycensus](https://walkerke.github.io/tidycensus/index.html) is a brand new and very handy package - note that you'll have to [request](http://api.census.gov/data/key_signup.html) a Census Data API key to run this code - so just you can just examine code below for now.
