...
```{r nhd, message=FALSE, warning=FALSE, error=FALSE, eval=FALSE}
...
```{r nlcd, message=FALSE, warning=FALSE, error=FALSE, eval=FALSE}
library(FedData)
library(raster)
...
```{r nlcd2, message=FALSE, warning=FALSE, error=FALSE, eval=FALSE}
NAvalue(NLCD) <-0 
plot(ThirdFork_alb$geometry, main="Elevation (m) in Third Fork Watershed", axes=T)
plot(NLCD, add=TRUE)
```