...
parking_near_parks <- st_join(bike_parking, parks, st_is_within_distance, dist =1609.34) # meters - miles conversion factor
parking_near_parks <- parking_near_parks %>%
  dplyr::distinct(fid)
plot(parks$geometry, col='green', axes=T)
plot(bike_racks$geometry, pch=2, cex=.5, col='black',add=T)
