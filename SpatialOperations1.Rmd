...
Let's grab a couple more sample data sets from [Open Data Durham](https://live-durhamnc.opendata.arcgis.com/). We'll's use [tidycensus](https://walkerke.github.io/tidycensus/index.html) census tracts as we did in previous module, and we'll join this to survey data by block group from [Open Data Durham](https://live-durhamnc.opendata.arcgis.com/) to evaluate survey point data at the block group level by census tract.  
...
survey_blkgrps %>%
  group_by(GEOID) %>%
  dplr::summarize(NQual = names(which.max(table(Neighb_Qual, exclude=NULL)))) %>%
...
survey_blkgrps %>% 
  ...
  group_by(Tract) %>%
  dplyr::summarize(NQual = names(which.max(table(NQual, exclude=NULL)))) %>%
  ggplot() + geom_sf(aes(fill=(NQual))) +
  ggtitle("Neighborhood Quality Perception \nby Census Tracts") + 
  theme_bw()  
```
