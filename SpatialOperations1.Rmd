...
* Familiarity with topological operations (spatial subsetting, proximity, aggregation, spatial joins) using `sf`
...
Now let's look at an example of some raster processing.  First, let's generate a watershed to use as our template, and as a first step we'll make use of the nice [nhrR](https://jsta.github.io/nhdR/) package by Joseph Stachelek.

```{r watershed stats, message=FALSE, warning=FALSE, error=FALSE}
